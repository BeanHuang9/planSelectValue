...
const downloadItems = ref(['下載為xls']);
// , '下載為PDF'
...
const isPopupVisibleTop = ref(false);
const isPopupVisibleCheck = ref(false);

const showPopup = () => {
	// Show the popup only if the currentTab is "是"
	if (currentTab.value === 1 || currentTab2.value === 4) {
		isPopupVisible.value = true;
	} else {
		// Close the popup if currentTab is not "是"
...
const closePopup = () => {
	isPopupVisible.value = false;
	isPopupVisibleTop.value = false;
	isPopupVisibleCheck.value = false;
};

const showConfirmationPopup = () => {
	isPopupVisibleTop.value = true;
};

const showCheckPopup = () => {
	isPopupVisibleCheck.value = true;
};
</script>
...
<template>
	<div class="py-16">
		<div>
			...
			<teleport to="body">
				<div v-if="isPopupVisible" class="popup-overlay">
					<div class="popup-content flex items-center justify-between w-2/3 m-auto" @click.stop>
						<div class="footerPopupItem">已選 3 項變動</div>
						<div class="footerPopupItem">
							<!-- Add a click event listener to the "確認送出" link -->
							<a
								href="#"
								@click="showConfirmationPopup"
								class="text-white bg-[#0087E0] hover:bg-blue-800 rounded-md text-sm px-7 py-2 me-2 mb-2 sendChanges"
								>確認送出</a
							>
						</div>
					</div>
			...
			<!-- check popup -->
			<teleport to="body">
				<div v-if="isPopupVisibleTop" class="popup-overlayTop text-center">
					<div class="popup-contentTop" @click.stop>
						<h3 class="text-lg">確認報到狀態變更</h3>
						<p class="py-8 px-3 leading-normal">您將所選的 3 筆預約，狀態變更成"是"<br />是否繼續？</p>

						<button
							@click="showCheckPopup"
							type="button"
							class="text-white bg-[#0087E0] hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium text-sm w-36 px-5 py-2 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-[#0087E0] focus:outline-none dark:focus:ring-blue-800"
						>
							是
						</button>
						<button
							type="button"
							class="w-36 py-2 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white border border-gray-200 hover:bg-gray-100 hover:text-[#FD6161] focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
							@click="closePopup"
						>
							取消
						</button>
			...
			<!-- check2 popup -->
			<teleport to="body">
				<div v-if="isPopupVisibleCheck" class="popup-overlayTop text-center">
					<div class="popup-contentTop" @click.stop>
						<h3 class="text-lg">確認預約狀態變更</h3>
						<p class="py-8 px-3 leading-normal">
							您將所選的 3 筆預約，狀態變更成"是"<br />是否繼續？<br />（ <span class="text-[#FD6161]">此狀態送出後無法再修改</span> ）
						</p>
		...
		<div class="w-3/4 m-auto tracking-wide">
			...
			<div class="relative shadow-md rounded-lg">
				<table class="w-full text-sm text-left rtl:text-right text-black dark:text-gray-400">
					<thead class="text-base text-black bg-[#CACACA] bg-opacity-30 dark:bg-gray-700 dark:text-gray-400">
						<tr>
							<th scope="col" class="px-3 py-4">訂單時間</th>
							<th scope="col" class="px-3 py-4">受檢時間</th>
							<th scope="col" class="px-3 py-4">姓名/生日</th>
							<th scope="col" class="px-3 py-4">公司</th>
							<th scope="col" class="px-3 py-4">方案名稱</th>
							<th scope="col" class="px-3 py-4">訂單狀態</th>
							<th scope="col" class="px-3 py-4 relative">
								預約確認
								<!-- <a href="#"
									><svg
										class="w-3 h-3 text-gray-800 dark:text-white absolute top-[42%] right-[15%]"
										aria-hidden="true"
										xmlns="http://www.w3.org/2000/svg"
										fill="currentColor"
										viewBox="0 0 16 10"
					...
					<tbody>
						<tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-[#CACACA] even:bg-opacity-10 border-b dark:border-gray-700 borderTopLine">
							<td class="px-2 py-4">2023-10-02-<br />8:20-8:40</td>
							<td class="px-2 py-4">2023-10-02-<br />8:20-8:40</td>
							<td class="px-3 py-4">林OO<br /><small>1989/06/04</small></td>
							<td class="px-3 py-4">亞東氣體</td>
							<td class="px-3 py-4 whitespace-nowrap dark:text-white">亞東氣體-<br />XXXXXXXX 方案Ａ</td>
							<td class="px-3 py-4">需要改期</td>
							<td class="px-3 py-4">
								<div class="flex justify-center">
									...
									>
										<input type="radio" id="radio-1" name="tabs" :value="1" v-model="currentTab" @change="showPopup()" />
										<label class="tab activeY" for="radio-1">是</label>
										<input type="radio" id="radio-2" name="tabs" :value="2" v-model="currentTab" />
										<label class="tab activeA" for="radio-2">-</label>
										<input type="radio" id="radio-3" name="tabs" :value="3" v-model="currentTab" />
										<label class="tab activeN" for="radio-3">否</label>
								...
								<div class="flex justify-center">
									...
									>
										<input
											...
											"
										/>
										<label class="tab activeY" for="radio-4">是</label>
										<input type="radio" id="radio-5" name="tabs2" @change="setCurrentTab2(5)" />
										<label class="tab activeA" for="radio-5">-</label>
										<input type="radio" id="radio-6" name="tabs2" @change="setCurrentTab2(6)" />
										<label class="tab activeN" for="radio-6">否</label>
										<span class="glider"></span>
									</div>
								</div>
							</td>
						...
						<tr class="cancelBox odd:bg-white odd:dark:bg-gray-900 even:bg-[#CACACA] even:bg-opacity-10 border-b dark:border-gray-700 borderTopLine">
							<td class="px-2 py-4">2023-10-02-<br />8:20-8:40</td>
							<td class="px-2 py-4">2023-10-02-<br />8:20-8:40</td>
							<td class="px-3 py-4">林OO<br /><small>1989/06/04</small></td>
							<td class="px-3 py-4">亞東氣體</td>
							<td class="px-3 py-4 whitespace-nowrap dark:text-white">亞東氣體-<br />XXXXXXXX 方案Ａ</td>
							<td class="px-3 py-4">預約取消</td>
							<td class="px-3 py-4">
								<div class="flex justify-center">Btn</div>
							</td>
							<td class="px-3 py-4">
...
tr.cancelBox {
	...
	opacity: 0.5; /* Set opacity to visually indicate the disabled state */
}
...
.tabsSwitch {
	...
	box-shadow: 0px 0px 13px 2px rgba(0, 0, 0, 0.3) inset;
...
input[id='radio-1']:checked ~ .activeY {
	color: #00c567;
}
input[id='radio-2']:checked ~ .activeA {
	color: rgba(0, 0, 0, 0.3);
}
input[id='radio-3']:checked ~ .activeN {
	color: #fd6161;
}

input[id='radio-4']:checked ~ .activeY {
	color: #00c567;
}
input[id='radio-5']:checked ~ .activeA {
	color: rgba(0, 0, 0, 0.3);
}
input[id='radio-6']:checked ~ .activeN {
	color: #fd6161;
}
...
.popup-overlay {
	position: fixed;
	bottom: 0; // Fix the popup at the bottom
	left: 0;
	width: 100%;
	background: #fff;
